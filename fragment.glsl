...
void main() {
  vec2 p = 2.0 * gl_FragCoord.xy / u_size.xy - 1.0;
  p.x *= u_size.x / u_size.y;  // square the pattern

  float color = 0.0;
  int id = int(u_screen.z) - 1;
  if (0 == 1) { }
  else if (id == 0) {
    color = min(a(p), min(b(p), min(c(p), min(d(p), min(e(p), f(p))))));
  }
  else if (id == 1) {
    color = min(b(p), c(p));
  }
  else if (id == 2) {
    color = min(a(p), min(b(p), min(d(p), min(e(p), g(p)))));
  }
  else if (id == 3) {
    color = min(a(p), min(b(p), min(c(p), min(d(p), g(p)))));
  }
  else if (id == 4) {
    color = min(b(p), min(c(p), min(f(p), g(p))));
    // b c f g
  }
  else if (id == 5) {
    color = min(a(p), min(c(p), min(d(p), min(f(p), g(p)))));
    // a c d f g
  }
  else if (id == 6) {
    color = min(a(p), min(c(p), min(d(p), min(e(p), min(f(p), g(p))))));
    // not b
  }
  else if (id == 7) {
    color = min(a(p), min(b(p), c(p)));
    // a b c
  }
  else if (id == 8) {
    color = min(a(p), min(b(p), min(c(p), min(d(p), min(e(p), min(f(p), g(p)))))));
    // a b c d e f g
  }
  else if (id == 9) {
    color = min(a(p), min(b(p), min(c(p), min(f(p), g(p)))));
    // not e or d
  }
  else if (id == 0xA) {
    color = min(a(p), min(b(p), min(c(p), min(e(p), min(f(p), g(p))))));
  }
  else if (id == 0xb) {
    color = min(c(p), min(d(p), min(e(p), min(f(p), g(p)))));
  }
  else if (id == 0xC) {
    color = min(a(p), min(f(p), min(e(p), d(p))));
  }
  else if (id == 0xd) {
    color = min(b(p), min(c(p), min(d(p), min(e(p), g(p)))));
  }
  else if (id == 0xE) {
    color = min(a(p), min(d(p), min(e(p), min(f(p), g(p)))));
  }
  else if (id == 0xF) {
    color = min(a(p), min(e(p), min(f(p), g(p))));
  }
  else {
    discard;
  }
